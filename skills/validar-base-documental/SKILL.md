...
PDFs legíveis sem XML correspondente são aceitos como evidência pendente: o cabeçalho pode alimentar a contagem e o montante indicativo de `unmatched_pdf_summary`, mas não autoriza planejamento nem análise de itens/créditos. Consulte os artefatos locais `03_SAIDAS/chaves-pendentes-xml.local.txt` e `03_SAIDAS/documentos-apenas-pdf.local.jsonl` para orientar o resgate do XML. Se o relatório mencionar valores não consolidados, explique que eles permanecem fora da base fiscal até o XML ser validado.
